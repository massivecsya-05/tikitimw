...
const Badge = ({ type }: { type: "sold_out" | "limited" | "free" }) => {
  const styles = {
    ...
    limited: "bg-amber-500 text-amber-950",
...
export const EventCard = ({ e }: { e: EventCardData }) => {
  ...
  return (
    <article className="group flex flex-col h-full animate-fade-up">
      <div className="relative overflow-hidden rounded-xl bg-gradient-card shadow-card border border-border/50 transition-all duration-150 ease-out hover:scale-[1.02] hover:shadow-glow flex flex-col h-full">
        <Link to={`/events/${e.id}`} className="block relative">
          <EventImage
            src={e.banner_url}
            alt={e.title}
            aspectClass="aspect-video"
            fallback={<span className="text-5xl">{cat?.emoji ?? "🎟️"}</span>}
          />
