...
export function detectMobileNetwork(phone: string): "tnm_mpamba" | "airtel_money" | null {
  ...
  let local = digits.startsWith("265") ? digits.slice(3) : digits;
  // International +265 format drops the trunk prefix (0); normalise to local 10-digit form.
  if (local.length === 9 && !local.startsWith("0")) local = `0${local}`;
  if (["0881", "0882", "0883", "0884", "0885"].some((p) => local.startsWith(p))) return "tnm_mpamba";
  if (["0999", "0977", "0978", "0979", "0990", "0991", "0992", "0888"].some((p) => local.startsWith(p))) {
    return "airtel_money";
  }
  if (local.startsWith("088")) return "tnm_mpamba";
  if (local.startsWith("099") || local.startsWith("097")) return "airtel_money";
  return null;
}
