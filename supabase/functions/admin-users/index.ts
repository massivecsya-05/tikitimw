    ...
    if (action === "delete") {
      ...
      if (error) {
        ...
        if (/foreign key|violates/i.test(raw)) {
          friendly = "Cannot delete user — related records still reference them. Reassign or remove those first.";
        } else if (/not.?found|no.?user/i.test(raw)) {
          friendly = "User not found — they may already be deleted. Refresh and try again.";
