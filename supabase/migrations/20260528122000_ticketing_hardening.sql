-- Add richer ticket tier controls (keep legacy sold for compatibility)
...
-- Store customer checkout details on orders (needed for delivery and anti-fraud checks)
...
-- Tickets table (UUID per ticket, QR data URL, one-time status)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.tickets (
  ...
  buyer_email text,
  buyer_phone text,
  qr_code text NOT NULL, -- base64/data-url image generated by edge function
...
-- Fraud prevention: max 4 tickets per phone per event (pending+paid)
...
-- Atomic scanner RPC using ticket UUID
...
-- Harden payment confirmation against overselling and keep both counters in sync
